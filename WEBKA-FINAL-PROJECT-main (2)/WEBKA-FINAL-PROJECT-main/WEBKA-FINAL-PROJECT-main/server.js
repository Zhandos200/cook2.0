...
app.get('/profile', requireAuth, async (req, res) => {
    try {
        ...
        // Find favorites for the logged-in user
        const favorites = await Favorite.find({ userId: req.session.userId });
        
        // Render profile.ejs, passing both the user and favorites data
        res.render('profile', { user, favorites, isAuthenticated: true });
    } catch (err) {
        console.error(err);
        res.redirect('/login');
    }
